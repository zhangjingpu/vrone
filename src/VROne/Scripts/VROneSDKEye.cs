...
public class VROneSDKEye : MonoBehaviour {
	...
	#region Properties
	/**
	 * Property is true if this eye is the left eye.
	 */
	...
	public float IPD {
		...
		set {
			...
			// dependent on the device the camera rect needs to be adjusted
			float cameraOffset = VROneSDKDevice.sharedInstance.cameraOffset;
			camera.pixelRect = new Rect(isLeftEye ? 0.0f : (Screen.width / 2.0f + cameraOffset), 0.0f, Screen.width / 2 - cameraOffset, Screen.height);
		}
	}
	...
	void Start () {
		...
		_isLeftEye = name == "VROneSDKEyeLeft";
